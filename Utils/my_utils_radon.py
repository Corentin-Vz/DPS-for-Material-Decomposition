...
#     sino_fft = torch.rfft(padded_sinogram) 
    sino_fft = torch.fft.rfft(padded_sinogram, dim=-1)
    
    # get filter and apply
    f = get_filter(padded_size, sinogram.device)
...
def get_HU_array(array, energy, spacing):
    ...
    # Units : 
    # spacing converts pixel^-1 to cm^-1
    # array in pixel^-1
    # energy in cm^2 g^-1
    # density in g cm^-3
    # HU : no unit
